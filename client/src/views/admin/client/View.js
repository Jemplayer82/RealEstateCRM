...
const View = () => {
  ...
  const clientData = useSelector(
    (state) => state?.clientCustomFiled?.data?.data,
  );
  ...
  const fetchData = async (i) => {
    setIsLoding(true);
    let response = await getApi("api/client/view/", param?.id);
  ...
  const handleDeleteClient = async (id) => {
    try {
      setIsLoding(true);
      let response = await deleteApi("api/client/delete/", id);
  ...
  useEffect(() => {
    dispatch(fetchClientCustomFiled());
  }, []);
